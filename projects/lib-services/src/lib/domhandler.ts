...
export class DomHandler {
  ...
  public static absolutePosition(
    ...
    // console.log('target.parent', target.parentElement.parentElement);
    // console.log('targetOffset', target.getBoundingClientRect());
    ...
    // const w = this.getOuterWidth(element, true);
    // console.log(w, elementOuterWidth);
    //
    // console.log('elementOuterHeight', elementOuterHeight);
    // console.log('elementOuterWidth', elementOuterWidth);
    // console.log('targetOuterHeight', targetOuterHeight);
    // console.log('targetOuterWidth', targetOuterWidth);
    ...
    if (!position || position === 'top' || position === 'bottom') {
      ...
      if (
        ...
        // right = targetOffset.right + windowScrollLeft;
      } else {
        left = targetOffset.left + windowScrollLeft;
      }

      // todo: fix the defect
      // @remark: patch for position:
      // chrome: top && align: right. unknown left offset -15px
      // safari: bottom && align: right. unknown left offset -16px

      // if ((targetOffset.top + targetOuterHeight + elementOuterHeight > viewport.height || position === 'top')
      //   && (targetOffset.left + elementOuterWidth > viewport.width || align === 'right')) {
      //   console.log(navigator.userAgent);
      //   if (navigator.userAgent.indexOf('Chrome') > 0) left += 15;
      // }
      ...
      if (
        targetOffset.right + elementOuterWidth > viewport.width ||
        position === 'left'
      ) {
        left = Math.max(
          0,
          targetOffset.left + windowScrollLeft - elementOuterWidth
        );
      } else {
        left = targetOffset.right + windowScrollLeft;
      }

      if (align === 'bottom')
        top = targetOffset.top - elementOuterHeight + targetOuterHeight;
      else top = targetOffset.top;
    }
  ...
  public static isElement(obj: any) {
    return typeof HTMLElement === 'object'
      ...
      : obj &&
      typeof obj === 'object' && true && obj.nodeType === 1 &&
...
//
// public static absolutePosition(element: any, target: any, position?: string, align?: string): void {
//   let elementDimensions = element.offsetParent
//     ? {width: element.offsetWidth, height: element.offsetHeight}
//     : this.getHiddenElementDimensions(element);
