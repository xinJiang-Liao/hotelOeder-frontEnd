...
export class BsModalMsgService {
  ...
  public show(title: string, text: string, options?: any): void {
    const componentRef = this.componentFactoryResolver.resolveComponentFactory(BsModalMsgComponent)
      .create(this.injector);

    const tplRef = (componentRef.instance as BsModalMsgComponent).tplMsg;
    if (tplRef) this.bsModalService.show(tplRef, {head: title, body: text}, options);
